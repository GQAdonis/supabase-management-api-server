...
func (oc *OrganizationCreate) AddProviderIDs(ids ...int) *OrganizationCreate {
	oc.mutation.AddProviderIDs(ids...)
	return oc
}

// AddProviders adds the "providers" edges to the Provider entity.
func (oc *OrganizationCreate) AddProviders(p ...*Provider) *OrganizationCreate {
	ids := make([]int, len(p))
	for i := range p {
		ids[i] = p[i].ID
	}
	return oc.AddProviderIDs(ids...)
}

// AddThirdPartyAuthIDs adds the "third_party_auths" edge to the ThirdPartyAuth entity by IDs.
func (oc *OrganizationCreate) AddThirdPartyAuthIDs(ids ...int) *OrganizationCreate {
	oc.mutation.AddThirdPartyAuthIDs(ids...)
	return oc
}

// AddThirdPartyAuths adds the "third_party_auths" edges to the ThirdPartyAuth entity.
func (oc *OrganizationCreate) AddThirdPartyAuths(t ...*ThirdPartyAuth) *OrganizationCreate {
	ids := make([]int, len(t))
	for i := range t {
		ids[i] = t[i].ID
	}
	return oc.AddThirdPartyAuthIDs(ids...)
}
...
func (oc *OrganizationCreate) createSpec() (*Organization, *sqlgraph.CreateSpec) {
	...
	if nodes := oc.mutation.ProvidersIDs(); len(nodes) > 0 {
	...
	if nodes := oc.mutation.ThirdPartyAuthsIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
