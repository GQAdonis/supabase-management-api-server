...
func (pq *ProviderQuery) QueryOrganization() *OrganizationQuery {
...
func (pq *ProviderQuery) WithOrganization(opts ...func(*OrganizationQuery)) *ProviderQuery {
	query := (&OrganizationClient{config: pq.config}).Query()
...
func (pq *ProviderQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Provider, error) {
	...
	if query := pq.withOrganization; query != nil {
		if err := pq.loadOrganization(ctx, query, nodes, nil,
			func(n *Provider, e *Organization) { n.Edges.Organization = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (pq *ProviderQuery) loadOrganization(ctx context.Context, query *OrganizationQuery, nodes []*Provider, init func(*Provider), assign func(*Provider, *Organization)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Provider)
	for i := range nodes {
		if nodes[i].organization_providers == nil {
			continue
		}
		fk := *nodes[i].organization_providers
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "organization_providers" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
