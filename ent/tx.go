...
func (tx *Tx) init() {
	...
	tx.CustomHostname = NewCustomHostnameClient(tx.config)
	tx.Function = NewFunctionClient(tx.config)
	tx.NetworkBan = NewNetworkBanClient(tx.config)
	tx.Organization = NewOrganizationClient(tx.config)
	tx.PgsodiumConfig = NewPgsodiumConfigClient(tx.config)
	tx.Project = NewProjectClient(tx.config)
	tx.Provider = NewProviderClient(tx.config)
	tx.Secret = NewSecretClient(tx.config)
	tx.ThirdPartyAuth = NewThirdPartyAuthClient(tx.config)
	tx.TypeScriptType = NewTypeScriptTypeClient(tx.config)
...
// applies a query, for example: AuthConfig.QueryXXX(), the query will be executed
