...
type OrganizationQuery struct {
	...
	withThirdPartyAuths *ThirdPartyAuthQuery
...
func (oq *OrganizationQuery) QueryProviders() *ProviderQuery {
	query := (&ProviderClient{config: oq.config}).Query()
...
// QueryThirdPartyAuths chains the current query on the "third_party_auths" edge.
func (oq *OrganizationQuery) QueryThirdPartyAuths() *ThirdPartyAuthQuery {
	query := (&ThirdPartyAuthClient{config: oq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := oq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (oq *OrganizationQuery) Clone() *OrganizationQuery {
	...
	return &OrganizationQuery{
		...
		withProviders:       oq.withProviders.Clone(),
		withThirdPartyAuths: oq.withThirdPartyAuths.Clone(),
...
func (oq *OrganizationQuery) WithProviders(opts ...func(*ProviderQuery)) *OrganizationQuery {
	query := (&ProviderClient{config: oq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	oq.withProviders = query
	return oq
}

// WithThirdPartyAuths tells the query-builder to eager-load the nodes that are connected to
// the "third_party_auths" edge. The optional arguments are used to configure the query builder of the edge.
func (oq *OrganizationQuery) WithThirdPartyAuths(opts ...func(*ThirdPartyAuthQuery)) *OrganizationQuery {
	query := (&ThirdPartyAuthClient{config: oq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	oq.withThirdPartyAuths = query
	return oq
}
...
func (oq *OrganizationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Organization, error) {
	var (
		...
		_spec       = oq.querySpec()
		loadedTypes = [2]bool{
			oq.withProviders != nil,
			oq.withThirdPartyAuths != nil,
		}
	)
	...
	if query := oq.withProviders; query != nil {
		if err := oq.loadProviders(ctx, query, nodes,
			func(n *Organization) { n.Edges.Providers = []*Provider{} },
			func(n *Organization, e *Provider) { n.Edges.Providers = append(n.Edges.Providers, e) }); err != nil {
			return nil, err
		}
	}
	if query := oq.withThirdPartyAuths; query != nil {
		if err := oq.loadThirdPartyAuths(ctx, query, nodes,
			func(n *Organization) { n.Edges.ThirdPartyAuths = []*ThirdPartyAuth{} },
			func(n *Organization, e *ThirdPartyAuth) { n.Edges.ThirdPartyAuths = append(n.Edges.ThirdPartyAuths, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (oq *OrganizationQuery) loadProviders(ctx context.Context, query *ProviderQuery, nodes []*Organization, init func(*Organization), assign func(*Organization, *Provider)) error {
	...
	for _, n := range neighbors {
		fk := n.organization_providers
		if fk == nil {
			return fmt.Errorf(`foreign-key "organization_providers" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "organization_providers" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (oq *OrganizationQuery) loadThirdPartyAuths(ctx context.Context, query *ThirdPartyAuthQuery, nodes []*Organization, init func(*Organization), assign func(*Organization, *ThirdPartyAuth)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Organization)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.withFKs = true
	query.Where(predicate.ThirdPartyAuth(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(organization.ThirdPartyAuthsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.organization_third_party_auths
		if fk == nil {
			return fmt.Errorf(`foreign-key "organization_third_party_auths" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "organization_third_party_auths" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
