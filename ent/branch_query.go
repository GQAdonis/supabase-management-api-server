...
type BranchQuery struct {
	...
	withParentProject *ProjectQuery
...
func (bq *BranchQuery) QueryProject() *ProjectQuery {
...
// QueryParentProject chains the current query on the "parentProject" edge.
func (bq *BranchQuery) QueryParentProject() *ProjectQuery {
	query := (&ProjectClient{config: bq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := bq.prepareQuery(ctx); err != nil {
			return nil, err
...
func (bq *BranchQuery) Clone() *BranchQuery {
	...
	return &BranchQuery{
		...
		withParentProject: bq.withParentProject.Clone(),
...
func (bq *BranchQuery) WithProject(opts ...func(*ProjectQuery)) *BranchQuery {
	query := (&ProjectClient{config: bq.config}).Query()
...
// WithParentProject tells the query-builder to eager-load the nodes that are connected to
// the "parentProject" edge. The optional arguments are used to configure the query builder of the edge.
func (bq *BranchQuery) WithParentProject(opts ...func(*ProjectQuery)) *BranchQuery {
	query := (&ProjectClient{config: bq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	bq.withParentProject = query
	return bq
}
...
func (bq *BranchQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Branch, error) {
	...
	if query := bq.withProject; query != nil {
		if err := bq.loadProject(ctx, query, nodes, nil,
			func(n *Branch, e *Project) { n.Edges.Project = e }); err != nil {
			return nil, err
		}
	}
	if query := bq.withParentProject; query != nil {
		if err := bq.loadParentProject(ctx, query, nodes, nil,
			func(n *Branch, e *Project) { n.Edges.ParentProject = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (bq *BranchQuery) loadProject(ctx context.Context, query *ProjectQuery, nodes []*Branch, init func(*Branch), assign func(*Branch, *Project)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Branch)
	for i := range nodes {
		fk := nodes[i].ProjectRef
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "project_ref" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (bq *BranchQuery) loadParentProject(ctx context.Context, query *ProjectQuery, nodes []*Branch, init func(*Branch), assign func(*Branch, *Project)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Branch)
	for i := range nodes {
		fk := nodes[i].ParentProjectRef
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "parent_project_ref" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (bq *BranchQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := bq.ctx.Fields; len(fields) > 0 {
		...
		if bq.withProject != nil {
			_spec.Node.AddColumnOnce(branch.FieldProjectRef)
		}
		if bq.withParentProject != nil {
			_spec.Node.AddColumnOnce(branch.FieldParentProjectRef)
		}
	}
